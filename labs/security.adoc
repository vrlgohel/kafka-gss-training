...
````
oc apply -f /configurations/clusters/production-ready-secured.yaml
````
...
We need to create users so that our clients can connect.
...
````
oc apply -f /configurations/users/secure-topic-reader.yaml
````
...
Now we need a user that can produce data to `lines`!
...
````
oc apply -f /configurations/users/secure-topic-writer.yaml
````
...
````
oc apply -f /configurations/applications/timer-producer-secured.yaml
````
...
````
oc apply -f /configurations/applications/log-consumer-secured.yaml
````

Check the log of `log-consumer` again.
